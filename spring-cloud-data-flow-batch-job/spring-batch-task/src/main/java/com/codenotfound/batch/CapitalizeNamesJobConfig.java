...
public class CapitalizeNamesJobConfig {
	...
	@Bean(name="capitalizeNamesJob")
	public Job convertNamesJob(JobBuilderFactory jobBuilders, StepBuilderFactory stepBuilders) throws IOException {
		return jobBuilders.get("capitalizeNamesJob").start(convertNamesStep(stepBuilders)).build();
	}

	@Bean
	public Step convertNamesStep(StepBuilderFactory stepBuilders) throws IOException {
	...
	@Bean
	public FlatFileItemWriter<Person> itemWriter() throws IOException {
		String personOutputPath = File.createTempFile("personOutputPath", ".out").getAbsolutePath();
		System.out.println(">> Output Path = "+personOutputPath);
...
//		flatFileItemWriter.setResource(new FileSystemResource("target/test-outputs/persons.txt"));
		flatFileItemWriter.setResource(new FileSystemResource(personOutputPath));
